...
export function meta({}: Route.MetaArgs) {
  return [
    ...
    {
      name: "description",
      content: "Spin the reels and hit the jackpot in our exciting slot machine!",
    },
  ];
}
        ...
        <div className="bg-black/50 rounded-lg p-4 mb-4">
          <div className="flex justify-center space-x-2 mb-2">
            {[0, 1, 2, 3, 4].map((reel) => (
              <div
                key={reel}
                className="bg-white rounded-lg w-16 h-20 flex items-center justify-center border-2 border-yellow-400"
              >
  ...
  useEffect(() => {
    if (user && session?.access_token && !loading) {
      ...
      if (wasAlreadyConnected) {
        console.log(
          "webSocket was already connected - joining slots room directly"
        );
        setIsConnected(true);
        websocketService.joinGame("slots", session.access_token);
      }
      ...
      console.log(
        "slots component waiting for auth - user:",
        !!user,
        "session:",
      ...
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center text-white">
          ...
          <p className="text-lg mb-8">
            Join our Discord community to start playing
          </p>
          <Link to="/">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="flex items-center space-x-1 sm:space-x-2 w-full">
                        ...
                        <input
                          ...
                          placeholder="0"
                        />
                        <span className="text-gray-400 flex-shrink-0 text-sm sm:text-base">
                          GC
                        </span>
                        <button
                      ...
                      <div className="grid grid-cols-5 gap-1 mt-2">
                        {[5, 10, 25, 50, 100].map((amt) => (
                          <button
                            key={amt}
                            onClick={() => updateBet(bet + amt)}
                            className="bg-gray-700 hover:bg-gray-600 text-white px-1 sm:px-2 py-1 rounded text-xs cursor-pointer"
                          >
                            +{amt}
                          </button>
                        ))}
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-1 sm:space-x-2">
                        <button
                          onClick={() =>
                            setPaylines(Math.max(1, paylines - 1))
                          }
                          className="bg-gray-700 hover:bg-gray-600 text-white px-2 sm:px-3 py-2 rounded transition-colors text-sm"
                        >
                          -1
                        </button>
                        <div className="bg-gray-800 border border-gray-600 px-2 sm:px-4 py-2 rounded text-center min-w-[60px] sm:min-w-[80px]">
                        ...
                        <button
                          onClick={() =>
                            setPaylines(Math.min(25, paylines + 1))
                          }
                          className="bg-gray-700 hover:bg-gray-600 text-white px-2 sm:px-3 py-2 rounded transition-colors text-sm"
                        >
                          +1
